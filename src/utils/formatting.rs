...
//! ## 模块位置历史
//!
//! 早期在 `desktop::components::formatting`，但 `truncate` / `format_local_unix_secs`
//! 本身零 GUI 依赖（前者只吃 `String`/`&str`/`char`，后者只吃 `i64` + 3 个 i18n key）。
//! 挪到 `util::formatting` 后：
//! - `app/` 和 `desktop/` 都能直接 `use crate::utils::formatting::*`；
//! - `desktop::components` 仍 `pub use crate::utils::formatting::{format_size, truncate}`
