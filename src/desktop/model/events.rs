...
/// 当前 view 的 `Render` 重绘。
///
/// 仅在桌面（GPUI）路径使用 —— 唯一调用方是 `desktop::drain_loop::spawn_drain_loop`
...
pub fn drain(model: &mut AppModel) -> bool {
    ...
    // `model.pending_ui_events`，由 `RootView::render` 排空 + 翻译 + 真正 push。
    //
    // "新版本" 场景用 `UIEvent::OpenLink` 携带 url —— `desktop::root::ui_event_to_notification`
