...
//! - [`register_key_bindings`] 在 `gpui_app::run` 启动时调一次。
//!
//! ## Key bindings 选择
//!
...
// `actions!` 宏在 [`crate::gpui_app`] 顶层 (mod.rs) 调用, 生成的 8 个 action 类型
// 位于 `gpui_app::*`。这里只 re-export 给 root.rs 用。
pub(super) use crate::gpui_app::{
