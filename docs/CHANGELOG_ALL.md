...
> Tasks 删除 toast 反馈 + Library notify watcher 重构。

- **Cargo.toml** `version = "0.3.5"` → `"0.3.6"`；`web-ui/package.json` 同步；
  `cli::args.rs::VERSION_STRING` 走 `env!("CARGO_PKG_VERSION")` 自动跟随

### Markdown 导出格式（11 commit + 3 docs）

新增 `ExportFormat::Markdown` 端到端支持 —— settings UI / web `FORMAT_OPTIONS` /
CLI `--format markdown` / library `.md` ext 过滤按钮 全链路打通。

- `85cf5c6` 2026-07-11 — **docs(spec)**: add Markdown export format design
- `d5b5455` 2026-07-11 — **docs(plan)**: add markdown export implementation plan
- `34c8e3a` 2026-07-11 — **docs(plan)**: switch MdExporter streaming to `write_all(...).as_bytes()`
- `5a2b5e9` 2026-07-11 — **feat(config)**: add `ExportFormat::Markdown` variant + `parse`/`as_lower` arms
- `4d99df6` 2026-07-11 — **docs(export)**: fix misleading Task-N reference in exporter.rs comments
- `26ec8ae` 2026-07-11 — **feat(export)**: `render_md` for Markdown export target
- `dfcacf1` 2026-07-11 — **feat(export)**: `write_chapter_files` emits `.md` under Markdown format
- `0f15919` 2026-07-11 — **feat(export)**: add `MdExporter` (front matter + H1 + TOC + chapter anchors)
- `a3d9358` 2026-07-11 — **feat(desktop)**: settings expose `ExportFormat::Markdown` as `'md'` option
- `8b64bce` 2026-07-11 — **feat(web)**: add `'markdown'` to `ExportFormat` union + `FORMAT_OPTIONS`
- `57f81ec` 2026-07-11 — **feat(library)**: add `'md'` ext filter button (desktop + web + core)
- `ff12c9a` 2026-07-11 — **feat(cli)**: document `--format` accepts markdown in help
- `4685278` 2026-07-11 — **fix(export)**: clippy nits in new markdown-export code
- `3ec193a` 2026-07-11 — **fix(review)**: address review findings — md i18n key + 2 missing tests

### URL 直接下载（SearchPage「下载链接」按钮）

跳过搜索结果页，从任意书源 URL 直接走选章下载。弹 Dialog 自动粘贴剪贴板 +
「粘贴」兜底按钮；按 origin / hash / port 严格匹配书源。

- `3bee268` 2026-07-11 — **docs(spec)**: relocate URL download entry to SearchPage header (v2)
- `b649f2c` 2026-07-12 — **feat(i18n)**: add `Search.url_download.*` keys for download-by-URL feature
- `3be6ade` 2026-07-12 — **feat(search)**: add `open_url_dialog` with clipboard auto-paste and origin matching
- `bdc21c2` 2026-07-12 — **feat(search)**: wire URL download button on SearchPage header
- `33102ea` 2026-07-12 — **fix(search)**: defer `open_range_dialog` via flag to avoid dialog-stack pop race
- `d18ee99` 2026-07-12 — **test(sources)**: lock down `match_source_by_url` for query/hash and port sensitivity

### Tasks 删除 toast 反馈

- `1b7aaa6` 2026-07-11 — **docs(spec)**: Tasks 删除成功/失败 toast 设计
- `a97b6ab` 2026-07-11 — **docs(plan)**: Tasks 删除 toast 实现步骤（3 任务 TDD）
- `c716051` 2026-07-11 — **feat(i18n)**: add Tasks `delete_task_ok` / `_still_running` / `_missing` toast strings
- `1a13b35` 2026-07-11 — **refactor(model)**: `delete_task` returns `DeleteTaskResult` enum
- `444d98b` 2026-07-11 — **fix(model)**: promote `mod tasks` to `pub(crate)` for `DeleteTaskResult` visibility
- `4287c5b` 2026-07-11 — **feat(desktop)**: Tasks `prompt_delete.on_ok` pushes success/warning toast
- `b4f0d59` 2026-07-11 — **docs(spec)**: correct `DeleteTaskResult` visibility mechanism

### Library 重构

- `1ea4580` 2026-07-12 — **refactor(library)**: drop `notify` watcher, add manual refresh button with loading
  —— 移除文件 watcher（打包后 Windows 上资源占用 + 资源刷新双触发问题），
  改为「刷新」按钮 + 加载态

### Fix

- `2794339` 2026-07-12 — **fix(config)**: default `download_path` fallback uses `'./downloads'` with prefix
  —— 与显式配置统一路径解析语义

### Docs

- `b4dce7f` 2026-07-11 — **docs**: bump Rust version badge to 1.95+
